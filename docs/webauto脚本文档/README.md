...
| `antexiadan-seckill-list.js` | 安特 homepage | 限时秒杀全量列表 → POST 入库 | [→](安特/antexiadan-限时秒杀列表.md) |
| `antexiadan-seckill-fetch.py` | pcapi 直连 | 同上（CLI，无需浏览器） | [→](安特/antexiadan-限时秒杀列表.md) |
